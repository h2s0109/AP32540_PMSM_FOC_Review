...
//---------------------------------------------------------------------------
/** Return 0 if the command has been executed successfully
 * Return -1 if in case of command error
 */
...
static boolean Ifx_Shell_writeResult(Ifx_Shell *shell, Ifx_SizeT Code)
{
    ...
    boolean   result = IfxStdIf_DPipe_write(shell->io, &Code, &length, IFX_CFG_SHELL_TIMEOUT);
...
boolean Ifx_Shell_showHelpSingle(Ifx_Shell_CommandListConst commandList, IfxStdIf_DPipe *io, boolean briefOnly, boolean singleCommand)
{
    ...
    while (command->commandLine != NULL_PTR)
    {
        ...
        while (*help != IFX_SHELL_NULL_CHAR)
        {
            ...
            else if (Ifx_Shell_matchCommand(&help, &matchSection))
            {   /* Parameter description found */
                isParam    = FALSE;
                isSyntax   = FALSE;
                sectionLevel  = 1;
            ...
            else if (Ifx_Shell_matchCommand(&help, &matchSubSection))
            {   /* Parameter description found */
                isParam    = FALSE;
                isSyntax   = FALSE;
				sectionLevel  = 2;
            ...
            if (isTitle)
            {
                if (sectionLevel == 1)
                {
                    IfxStdIf_DPipe_print(io, "%s=================================================="ENDL, space);
                    IfxStdIf_DPipe_print(io, "%s ", space);
                    IfxStdIf_DPipe_write(io, (void *)help, &count, IFX_CFG_SHELL_TIMEOUT);
                    IfxStdIf_DPipe_print(io, "%s=================================================="ENDL, space);
                }
...
{
    Ifx_Shell_CommandListConst commandList = NULL_PTR;
...
void Ifx_Shell_prefill(Ifx_Shell *shell, const char* text)
{
    Ifx_Shell_CmdLine *Cmd         = &shell->cmd;
    char              *cmdStr      = shell->locals.cmdStr;
    Ifx_SizeT          i;           /* Loop variables */
...
Ifx_Shell_SpecialKey Ifx_Shell_process(Ifx_Shell *shell)
{
    ...
    else
    {
        ...
        for (i = 0; i < count; i++)
        {
            ...
            case '\r':
                ...
                if (Cmd->length < IFX_CFG_SHELL_CMD_LINE_SIZE)
                {
                    ...
                    if (Cmd->historyAdd != FALSE)
                    {
                        ...
                        result = Ifx_Shell_SpecialKey_cr;
                    }

                    if (shell->control.execute == 1)
                    {
                        /* Execute command */
                        if (Ifx_Shell_execute(shell, cmdStr) == 0)
                        {
                        	result = Ifx_Shell_SpecialKey_ok;
                        }
                    }
                }
			...
			/* CTRL +C key sequence in terminal */
			case '\x03':
				/* Ignore previous characters and return 0x03 */
                result = Ifx_Shell_SpecialKey_ctrlC;

				break;
            /* Escape character */
            case '\x1B':       /*'\x1B': */
                shell->locals.cmdState = Ifx_Shell_CmdState_escape;
                break;

            /* '[' - check to see if this is second part of an escape sequence */
            case '[':

                if (shell->locals.cmdState == Ifx_Shell_CmdState_escape)
                {
                    /* ESC [ pressed */
                    shell->locals.cmdState = Ifx_Shell_CmdState_escapeBracket;
                }
                else
                {
                    NormalKeyPress = TRUE;
            ...
            case 'D':
                ...
                if (shell->locals.cmdState == Ifx_Shell_CmdState_escapeBracket)
                {
                    /* Process arrow keys */
            ...
            case '4':
                ...
                if (shell->locals.cmdState == Ifx_Shell_CmdState_escapeBracket)
                {
                    ...
                    shell->locals.cmdState      = Ifx_Shell_CmdState_escapeBracketNumber;
                }
                else
                {
                    NormalKeyPress = TRUE;
            ...
            case '~':
                ...
                if (shell->locals.cmdState == Ifx_Shell_CmdState_escapeBracketNumber)
                {
...
boolean Ifx_Shell_matchCommand(pchar *argsPtr, pchar *match)
{
    boolean result         = FALSE;
    pchar   savedArguments = *argsPtr;
    pchar   savedMatch     = *match;
    char    buffer0[IFX_CFG_SHELL_CMD_LINE_SIZE];
    char    buffer1[IFX_CFG_SHELL_CMD_LINE_SIZE];
...
Ifx_Shell_ExecuteCode Ifx_Shell_execute(Ifx_Shell *shell, pchar commandLine)
{
	Ifx_Shell_ExecuteCode result = Ifx_Shell_ExecuteCode_notFound;
    pchar                      args         = NULL_PTR;
    ...
    if (shellCommand != NULL_PTR)
    {
        IFX_ASSERT(IFX_VERBOSE_LEVEL_ERROR, shellCommand->call != FALSE);
        if (shellCommand->call(args, shellCommand->data, shell->io) != FALSE)
        {
            ...
            result = Ifx_Shell_ExecuteCode_ok;
        }
        else
        {
            if (shell->control.sendResultCode != 0)
            ...
            {}
                result = Ifx_Shell_ExecuteCode_cmdError;
        }
    }
    else
    {
        if (commandLine[0] != IFX_SHELL_NULL_CHAR)
        {
            ...
            {}
            result = Ifx_Shell_ExecuteCode_notFound;
        }
        else
        {
            result = Ifx_Shell_ExecuteCode_ok;
