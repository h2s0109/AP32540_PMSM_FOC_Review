...
#if !defined(IFX_CFG_SHELL_TIMEOUT)
#define IFX_CFG_SHELL_TIMEOUT (IFX_CFG_STDIF_DPIPE_TIMEOUT) /**<\brief Default shell Tx timeout */
#endif
...
//----------------------------------------------------------------------------------------
    typedef enum
    {
        Ifx_Shell_ExecuteCode_ok              = 0x00000000,  /** \brief Command found and executed without error  */
        Ifx_Shell_ExecuteCode_cmdError        = 0x00000001,  /** \brief Command found and executed with error  */
        Ifx_Shell_ExecuteCode_notFound        = 0xFFFFFFFF,  /** \brief Command not */
        Ifx_Shell_ExecuteCode_okNoHistory         = 0x00010000,  /** \brief Command found and executed without error  */
        Ifx_Shell_ExecuteCode_cmdErrorNoHistory   = 0x00010001,  /** \brief Command found and executed with error  */
    }Ifx_Shell_ExecuteCode;
...
typedef enum
{
    Ifx_Shell_CmdState_normal,                 /**< \brief Normal - no special sequences being processed */
    Ifx_Shell_CmdState_escape,                 /**< \brief Escape character has been pressed */
    Ifx_Shell_CmdState_escapeBracket,         /**< \brief ESC [ pressed */
    Ifx_Shell_CmdState_escapeBracketNumber   /**< \brief ESC [ (number) pressed */
} Ifx_Shell_CmdState;
...
typedef struct
{
    uint16 showPrompt : 1;
    uint16 enabled : 1;
    uint16 execute : 1; /* If set, Ifx_Shell_execute() is called within Ifx_Shell_process(). Default */
...
typedef struct
{
    ...
    boolean 				   execute;     							 /**< \brief If set, Ifx_Shell_execute() is called within Ifx_Shell_process(). Default */
} Ifx_Shell_Config;
...
IFX_EXTERN Ifx_Shell_ExecuteCode Ifx_Shell_execute(Ifx_Shell *shell, pchar commandLine);

/** Set the config default parameter
 *
 * \param config Pointer to the configuration structure to be initialized
 */
IFX_EXTERN void Ifx_Shell_initConfig(Ifx_Shell_Config *config);
...
typedef enum
{
	Ifx_Shell_SpecialKey_null 	= 0,
	Ifx_Shell_SpecialKey_cr 		= '\r',
	Ifx_Shell_SpecialKey_ctrlC 	= '\x03',
	Ifx_Shell_SpecialKey_ok = 0x10000			/**< Command has been executed successfully */
}Ifx_Shell_SpecialKey;

#define IFX_SHELL_KEY_NULL (Ifx_Shell_SpecialKey_null)
#define IFX_SHELL_KEY_CR (Ifx_Shell_SpecialKey_cr)
#define IFX_SHELL_KEY_CTRL_C (Ifx_Shell_SpecialKey_ctrlC)
#define IFX_SHELL_KEY_OK (Ifx_Shell_SpecialKey_ok) /* Command has been executed successfully */
 ...
 *
 * \param shell Pointer to the \ref Ifx_Shell object
 * \retval IFX_SHELL_CARRIAGE_RETURN if the ENTER is pressed (\n or \r) and command line not empty. Command can be read from the command history entry 0
 * \retval IFX_SHELL_CTRL_C if CTRL + C is pressed
 * \retval else IFX_SHELL_NULL
 */
IFX_EXTERN Ifx_Shell_SpecialKey Ifx_Shell_process(Ifx_Shell *shell);

/** Prefil the shell command line and echo to the console if configured
 * \param text Text to be prefilled in the command line
 */
IFX_EXTERN void Ifx_Shell_prefill(Ifx_Shell *shell, const char* text);
...
IFX_EXTERN boolean Ifx_Shell_matchCommand(pchar *argsPtr, pchar *match);
